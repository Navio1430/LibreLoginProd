...
public class AuthenticPremiumProvider implements PremiumProvider {
    ...
    public AuthenticPremiumProvider(AuthenticLibreLogin<?, ?> plugin) {
        this.plugin = plugin;
        userCache = Caffeine.newBuilder().expireAfterWrite(10, TimeUnit.MINUTES).build();
        ...
        // fetchers.add(this::getUserFromAshcon); //Momentarily disabled, as it's unreliable. See
        // https://github.com/Electroid/mojang-api/issues/79
    }
                        ...
                        x -> {
                            for (int i = 0; i < fetchers.size(); i++) {
                                ...
                                try {
                                    ...
                                    plugin.getLogger()
                                            .debug(
                                                    "Unexpected exception while fetching premium"
                                                            + " user "
                                                            + finalName,
                                                    e);
                                    if (i == fetchers.size() - 1) {
                                        exceptionToThrow[0] =
                                                new PremiumException(
                                                        PremiumException.Issue.UNDEFINED, e);
                                    }
                                }
                            }
                            return null;
    ...
    private PremiumUser getUserFromAshcon(String name) throws PremiumException {
        try {
            ...
            var connection =
                    (HttpURLConnection)
                            new URL("https://api.ashcon.app/mojang/v2/user/" + name)
                                    .openConnection();
            ...
            switch (connection.getResponseCode()) {
                case 200 -> {
                    var data =
                            AuthenticLibreLogin.GSON.fromJson(
    ...
    private PremiumUser getUserFromPlayerDB(String name) throws PremiumException {
        try {
            ...
            var connection =
                    (HttpURLConnection)
                            new URL("https://playerdb.co/api/player/minecraft/" + name)
                                    .openConnection();
            connection.setConnectTimeout(5000);
            connection.setReadTimeout(5000);
            ...
            switch (connection.getResponseCode()) {
                case 200 -> {
                    ...
                    return new PremiumUser(
                            UUID.fromString(id), username, username.equalsIgnoreCase(name));
                }
                case 400 -> {
                    return null;
                }
    ...
    private PremiumUser getUserFromMinetools(String name) throws PremiumException {
        try {
            ...
            var connection =
                    (HttpURLConnection)
                            new URL("https://api.minetools.eu/uuid/" + name).openConnection();
            connection.setConnectTimeout(5000);
            connection.setReadTimeout(5000);
                ...
                case 200 -> {
                    ...
                    if (rawId == null || rawId.isJsonNull()) {
                        ...
                        if (errorMessage.equals("Invalid UUID or nickname.")) {
                            ...
                            throw new PremiumException(
                                    PremiumException.Issue.UNDEFINED, errorMessage);
                        }
                    }
