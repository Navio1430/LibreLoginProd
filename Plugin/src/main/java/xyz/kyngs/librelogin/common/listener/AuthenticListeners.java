...
public class AuthenticListeners<Plugin extends AuthenticLibreLogin<P, S>, P, S> {
    ...
    protected PreLoginResult onPreLogin(String username, InetAddress address) {
        ...
        try {
            ...
            var message =
                    switch (e.getIssue()) {
                        case THROTTLED ->
                                plugin.getMessages().getMessage("kick-premium-error-throttled");
                        default -> {
                            plugin.getLogger()
                                    .error(
                                            "Encountered an exception while communicating with the"
                                                    + " Mojang API!");
                            e.printStackTrace();
        ...
        if (mojangData == null) {
            // A user with this name does not exist in the Mojang database. It is impossible for
            ...
            if (user.getPremiumUUID() != null) {
                // We will have to encrypt, otherwise someone could forcefully disable other user's
                // premium autologin
                return new PreLoginResult(PreLoginState.FORCE_ONLINE, null, user);
            }
        } else {

            // A user with this name exists in the Mojang database, we need to figure out whether to
            // encrypt
            ...
            if (user == null) {
                ...
                if (byName != null && !user.equals(byName)) {
                    // A user with this name already exists, however, it is not the same user as the
                    // premium one.
                ...
                if (!mojangData.reliable()) {
                    plugin.getLogger()
                            .warn(
                                    "User %s has probably changed their name. Data returned from Mojang API is not reliable, faking a new one using the current nickname."
                                            .formatted(username));
    ...
    private PreLoginResult handleProfileConflict(User conflicting, User conflicted) {
        return switch (ProfileConflictResolutionStrategy.valueOf(
                plugin.getConfiguration()
                        .get(ConfigurationKeys.PROFILE_CONFLICT_RESOLUTION_STRATEGY))) {
            case BLOCK ->
                    new PreLoginResult(
                            PreLoginState.DENIED,
                            plugin.getMessages()
     ...
     * @param generate True if a new user should be generated if the user doesn't exist, false
     *     otherwise.
     * @param ip The IP address of the user.
     * @return The validated user, or null if the user doesn't exist and {@code generate} is false.
     * @throws InvalidCommandArgument If the username is invalid or there are other validation
     *     issues.
     */
    private User checkAndValidateByName(
        ...
        if (user != null) {
            ...
            var minLength =
                    plugin.getConfiguration().get(ConfigurationKeys.MINIMUM_USERNAME_LENGTH);
            ...
            if (ipLimit > 0) {
                var ipCount =
                        plugin.getDatabaseProvider()
                                .getByIP(ip.getHostAddress())
                                .size(); // Ideally, this should be a count query, but I'm too lazy
                // to implement that and the performance impact is
                // negligible.
                ...
                if (ipCount >= ipLimit) {
                    throw new InvalidCommandArgument(
                            plugin.getMessages()
                                    .getMessage(
                                            "kick-ip-limit", "%limit%", String.valueOf(ipLimit)));
                }
            }
            ...
            if (premiumUser != null
                    && premiumUser.reliable()
                    && plugin.getConfiguration().get(ConfigurationKeys.AUTO_REGISTER)) {
                ...
                if (premiumUser != null && !premiumUser.reliable()) {
                    plugin.getLogger()
                            .warn(
                                    "The premium data for %s is not reliable, the user may not have the same name capitalization as the premium one. It is not safe to auto-register this user. Switching to offline registration!"
                                            .formatted(username));
                }
                user =
                        new AuthenticUser(
